...
#### [同构树](https://blog.csdn.net/weixin_43088751/article/details/104079228)
...
bool dfs(TreeNode* T1, TreeNode* T2) {
    if (!T1 && !T2)      // 如果两个树都为空
        ...
        return false;
    else if (T1->val != T2->val)  // 如果节点内的数据不一致
        return false;
    else   // 两棵同构树要么左节点=左节点，要么左节点=右节点
        return (dfs(T1->left, T2->left) && dfs(T1->right, T2->right)) || 
               (dfs(T1->left, T2->right) && dfs(T1->left, T2->right));
}
```
