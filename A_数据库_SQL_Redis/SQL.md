...
第二范式：非主属性必须完全依赖于候选码，不能部分依赖于候选码

- （sno,cno,姓名,score,系名,系主任）候选码是（sno,cno），name部分依赖于sno，因此不满足2NF，应该拆表，即（sno,姓名,系名，系主任）（sno,cno,score）

第三范式：非主属性不能传递依赖于候选码
- （sno,姓名,系名,系主任），“系主任”依赖于“系名”（非候选码），“系名”依赖于“sno”（候选码），那么，“系主任”（非主属性）传递依赖于“sno”（候选码），不符合3NF。应该拆表，即：（sno,姓名,系名）（系名,系主任）
